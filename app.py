...
from mainframe import generate_test_cases, upload_test_cases
...
def generate():
    try:
        ...
        output_file = generate_test_cases("temp_input.xlsx")
        count = len(pd.read_excel(output_file))
        return jsonify({'status': 'success', 'count': count, 'filename': os.path.basename(output_file)}), 200
...
def upload():
    try:
        ...
        upload_count, error_count = upload_test_cases("temp_output.xlsx", org, proj, pat, plan_name, suite_name)
